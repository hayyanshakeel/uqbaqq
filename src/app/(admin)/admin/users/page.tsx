...
// FIX: Added parseISO to the import list
import { format, startOfMonth, addMonths, lastDayOfMonth, differenceInMonths, isValid, parse, isAfter, parseISO } from 'date-fns';
...
async function getUsers(): Promise<User[]> {
    ...
    const users = await Promise.all(snapshot.docs.map(async (doc) => {
        ...
        return {
            ...
            joined: format(parseISO(data.joined), 'dd/MM/yyyy'),
...
export default async function UsersPage() {
    ...
    // --- SERVER ACTIONS ---
    async function addUserAction(formData: FormData) { 'use server'; /* ... logic ... */ }
    async function deleteUserAction(userId: string) { 'use server'; /* ... logic ... */ }
    async function updateUserAction(userId: string, formData: FormData) { 'use server'; /* ... logic ... */ }
    async function markAsDeceasedAction(userId: string, formData: FormData) { 'use server'; /* ... logic ... */ }
    async function sendPaymentLinkAction(userId: string) { 'use server'; /* ... logic ... */ }
    async function reverseLastPaymentAction(userId: string) { 'use server'; /* ... logic ... */ }
    async function recalculateBalanceUntilDateAction(userId: string, formData: FormData) { 'use server'; /* ... logic ... */ }
    async function getPendingBillsForUserAction(userId: string): Promise<Bill[]> { 'use server'; /* ... logic ... */ }
    async function markBillAsPaidAction(userId: string, billId: string, billAmount: number) { 'use server'; /* ... logic ... */ }
