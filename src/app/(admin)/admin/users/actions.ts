...
// --- *** FINAL, MOST ROBUST BULK ADD ACTION *** ---
export async function bulkAddUsersAction(userData: string) {
    ...
    for (const [index, line] of lines.entries()) {
        // Skip any empty lines in the text box
        if (!line.trim()) continue;

        const parts = line.split(',').map(v => v.trim());
        
        if (parts.length < 5) {
            errors.push(`Line ${index + 1}: Not enough data. Expected 5 columns, but got ${parts.length}.`);
            errorCount++;
            continue;
        }

        const [name, email, phone, password, joiningDate] = parts;

        try {
            if (!name || !email || !phone || !password || !joiningDate) {
                throw new Error("Missing required fields. Each line must have name, email, phone, password, and joining_date.");
            }
            if (password.length < 6) {
                throw new Error("Password must be at least 6 characters long.");
            }
            const joiningDateObj = parse(joiningDate, 'yyyy-MM-dd', new Date());
            if (!isValid(joiningDateObj)) {
                throw new Error(`Invalid joining_date format: "${joiningDate}". Expected YYYY-MM-DD.`);
            }
            ...
            try {
                ...
                if (error.code === 'auth/user-not-found') {
                    // This is the expected case, so we can proceed to create the user.
                } else {
                    // This catches other errors, like if the user already exists.
                    throw error;
                }
            }
            ...
            await adminDb.collection('users').doc(userRecord.uid).set({
                ...
                email,
                status: 'pending',
                joined: joiningDateObj.toISOString(),
                totalPaid: 0,
                pending: totalDues,
            });
            successCount++;
